...
public class AuthService {
  ...
  public LoginResponse login(LoginRequest request) {
    ...
    if (!passwordEncoder.matches(request.password(), user.getPassword())) {
      throw new ResponseStatusException(
          HttpStatus.UNAUTHORIZED, "Identifiants invalides (E-mail ou mot de passe incorrect)");
    }
    ...
    String token =
        jwtTokenProvider.generateToken(String.valueOf(user.getId()), user.getRole().name());
  ...
  public void verifyAccount(VerifyRequest request) {
    User user =
        userRepository
            .findByVerificationToken(request.token())
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Token de vérification invalide ou déjà utilisé"));
    ...
    User user =
        userRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "Token de réinitialisation invalide ou expiré"));
