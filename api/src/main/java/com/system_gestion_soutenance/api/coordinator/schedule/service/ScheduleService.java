...
public class ScheduleService {
  ...
  public Map<String, Map<String, Object>> saveSchedule(Map<String, Map<String, Object>> schedule) {
    ...
    for (Map.Entry<String, Map<String, Object>> entry : schedule.entrySet()) {
      ...
      if (data.containsKey("roomId") && data.get("roomId") != null) {
        Room room =
            roomRepository
                .findById(toLong(data.get("roomId")))
                .orElseThrow(
                    () ->
                        new ResponseStatusException(
                            HttpStatus.BAD_REQUEST, "Salle introuvable: " + data.get("roomId")));
        slot.setRoom(room);
      }
    ...
    DefenseSettings settings =
        defenseSettingsRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Paramètres de soutenance non trouvés"));
    ...
    while (!current.isAfter(ds.getEndDate())) {
      for (Room room : rooms) {
        ...
        while (time.plusMinutes(slotDuration).isBefore(endTime)
          ...
          approvedProjects.stream()
              .filter(p -> !assignedProjects.contains(p.getId()))
              .filter(p -> getStudentCountForProject(p.getId()) <= room.getCapacity())
              .findFirst()
              .ifPresent(
                  project -> {
  ...
  public void publish(Long defenseSessionId) {
    ...
    if (ds.getStatus().name().equals("ACTIVE")) {
      ds.setStatus(
          com.system_gestion_soutenance.api.admin.defensesession.entity.DefenseSessionStatus
              .SCHEDULED);
    ...
    SlotAssignment slot =
        slotAssignmentRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Créneau de soutenance non trouvé"));
