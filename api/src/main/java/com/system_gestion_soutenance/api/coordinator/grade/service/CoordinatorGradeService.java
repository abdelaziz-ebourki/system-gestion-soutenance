...
public class CoordinatorGradeService {
  ...
  public CoordinatorGradeService(
  ...
  public List<Map<String, Object>> getGrades() {
    ...
    for (Jury jury : juryRepository.findAll()) {
      ...
      List<Map<String, Object>> individualScores =
          buildIndividualScores(jury.getMembers(), evaluations);
  ...
  private Map<String, Integer> resolveCoefficients(Long defenseSessionId) {
    ...
    return defenseSessionRepository
  ...
  private List<Map<String, Object>> buildIndividualScores(
      List<JuryMember> members, List<Evaluation> evaluations) {
  ...
  private Double computeWeightedScore(
      List<JuryMember> members, List<Evaluation> evaluations, Map<String, Integer> coefficients) {
    ...
    for (JuryMember member : members) {
      Evaluation eval =
          evaluations.stream()
              .filter(
                  e -> e.getTeacherId().equals(member.getTeacher().getId()) && e.getScore() != null)
              .findFirst()
              .orElse(null);
      if (eval != null) {
