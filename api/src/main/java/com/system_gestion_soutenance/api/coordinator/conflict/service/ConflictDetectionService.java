...
public class ConflictDetectionService {
  ...
  public List<Map<String, Object>> validate(
      Map<String, Map<String, Object>> proposedSchedule, String defenseSessionId) {
  ...
  private List<Map<String, Object>> runAllChecks(
      Map<String, Map<String, Object>> schedule, String defenseSessionId) {
  ...
  private List<Map<String, Object>> checkProjectAlreadyScheduled(
      ...
      if (seen.contains(key)) {
        conflicts.add(
            createConflict(
                "slot_occupied",
                "error",
                "Un autre projet occupe déjà ce créneau (date: "
                    + date
                    + ", salle: "
                    + roomId
                    + ", horaire: "
                    ...
                    + ")",
                slotId,
                "Choisissez une autre date, salle ou horaire"));
      }
      seen.add(key);
    }
    return conflicts;
      ...
      if (studentCount > room.getCapacity()) {
        conflicts.add(
            createConflict(
                "room_capacity",
                "error",
                "Capacité de la salle insuffisante: "
                    + studentCount
                    + " étudiants pour "
                    + room.getCapacity()
                    + " places",
                slotId,
                "Choisissez une salle plus grande ou réduisez la taille du groupe"));
      }
    }
    return conflicts;
  }

  private List<Map<String, Object>> checkDateOutOfBounds(
      Map<String, Map<String, Object>> schedule, String defenseSessionId) {
    List<Map<String, Object>> conflicts = new ArrayList<>();
    if (defenseSessionId == null) return conflicts;

    DefenseSession ds =
        defenseSessionRepository.findById(Long.valueOf(defenseSessionId)).orElse(null);
        ...
        if (date.isBefore(ds.getStartDate()) || date.isAfter(ds.getEndDate())) {
          conflicts.add(
              createConflict(
                  ...
                  "La date "
                      + dateStr
                      + " est en dehors de la période autorisée ("
                      + ds.getStartDate()
                      + " – "
                      + ds.getEndDate()
                      + ")",
                  slotId,
                  "Choisissez une date entre " + ds.getStartDate() + " et " + ds.getEndDate()));
        }
  ...
  private List<Map<String, Object>> checkTeacherDoubleBooked(
      Map<String, Map<String, Object>> schedule) {
    List<Map<String, Object>> conflicts = new ArrayList<>();
  ...
  private List<Map<String, Object>> checkSupervisorConflict(
      Map<String, Map<String, Object>> schedule) {
    List<Map<String, Object>> conflicts = new ArrayList<>();
  ...
  private List<Map<String, Object>> checkBreakInterval(
      Map<String, Map<String, Object>> schedule, String defenseSessionId) {
    List<Map<String, Object>> conflicts = new ArrayList<>();
    ...
    for (Map.Entry<String, List<Map.Entry<String, Map<String, Object>>>> group :
        byDateRoom.entrySet()) {
      ...
      for (int i = 1; i < slots.size(); i++) {
        ...
        try {
          long gap =
              ChronoUnit.MINUTES.between(LocalTime.parse(prevTime), LocalTime.parse(currTime));
          if (gap < breakDuration) {
            conflicts.add(
                createConflict(
                    "break_violation",
                    "warning",
                    "Intervalle insuffisant entre les créneaux: "
                        + gap
                        + " min au lieu de "
                        + breakDuration
                        + " min",
  ...
  private List<Map<String, Object>> checkTeacherUnavailable(
      Map<String, Map<String, Object>> schedule) {
    List<Map<String, Object>> conflicts = new ArrayList<>();
  ...
  private int getStudentCountForProject(String projectId) {
    ...
    for (var g : groups) {
      if (g.getStudents() != null && !g.getStudents().isEmpty()) return g.getStudents().size();
    }
    Project project = projectRepository.findById(Long.valueOf(projectId)).orElse(null);
    if (project != null && project.getStudents() != null) return project.getStudents().size();
    return 0;
  }
  ...
  private Map<String, Object> createConflict(
      String type, String severity, String message, String slot, String resolution) {
