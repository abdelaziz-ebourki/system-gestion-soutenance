  ...
  public Map<String, Object> create(CreateJuryRequest request) {
    Project project =
        projectRepository
            ...
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Projet introuvable"));
  ...
  public Map<String, Object> update(Long id, Map<String, Object> updates) {
    Jury jury =
        juryRepository
            .findById(id)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Jury non trouvé"));

    if (updates.containsKey("projectId")) {
      Project project =
          projectRepository
              .findById(Long.parseLong((String) updates.get("projectId")))
              .orElseThrow(
                  () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Projet introuvable"));
      jury.setProject(project);
    }
    ...
    if (updates.containsKey("members")) {
      ...
      for (Map<String, String> m : memberData) {
        Teacher teacher =
            teacherRepository
                .findById(Long.parseLong(m.get("teacherId")))
                .orElseThrow(
                    () ->
                        new ResponseStatusException(
                            HttpStatus.BAD_REQUEST,
