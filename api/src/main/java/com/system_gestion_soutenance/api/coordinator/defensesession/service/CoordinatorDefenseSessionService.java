...
public class CoordinatorDefenseSessionService {
  ...
  private static final Map<DefenseSessionStatus, Set<DefenseSessionStatus>> VALID_TRANSITIONS =
      Map.of(
          DefenseSessionStatus.DRAFT,
              Set.of(DefenseSessionStatus.ACTIVE, DefenseSessionStatus.SCHEDULED),
          DefenseSessionStatus.ACTIVE,
              Set.of(DefenseSessionStatus.SCHEDULED, DefenseSessionStatus.COMPLETED),
  ...
  public CoordinatorDefenseSessionService(
  ...
  public DefenseSession create(CreateDefenseSessionRequest request) {
    ...
    ds.setStatus(
        request.status() != null ? parseStatus(request.status()) : DefenseSessionStatus.DRAFT);
    ...
    ds.setSubmissionDeadline(
        request.submissionDeadline() != null
            ? LocalDate.parse(request.submissionDeadline())
            : null);
    ...
    if (request.juryRoleTemplateId() != null) {
      JuryRoleTemplate template =
          juryRoleTemplateRepository
              .findById(request.juryRoleTemplateId())
              .orElseThrow(
                  () ->
                      new ResponseStatusException(
                          HttpStatus.BAD_REQUEST, "Template de rôle jury introuvable"));
    ...
    } else if (request.juryRoleTemplateId() != null
        && ds.getJuryRoleTemplate() != null
        && ds.getJuryRoleTemplate().getRoles() != null) {
      ds.setEvaluationCoefficients(
          ds.getJuryRoleTemplate().getRoles().stream()
    ...
    DefenseSession ds =
        defenseSessionRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Session de soutenance non trouvée"));

    DefenseSessionStatus newStatus =
        request.status() != null ? parseStatus(request.status()) : ds.getStatus();
  ...
  private void validateTransition(DefenseSessionStatus from, DefenseSessionStatus to) {
    ...
    if (allowed == null || !allowed.contains(to)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Transition de statut invalide: " + from + " → " + to);
    }
  }
