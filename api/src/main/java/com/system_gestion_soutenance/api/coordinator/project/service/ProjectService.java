...
public class ProjectService {
  ...
  public Map<String, Object> create(CreateProjectRequest request) {
    Teacher supervisor =
        teacherRepository
            .findById(request.supervisorId())
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Encadrant introuvable"));
  ...
  public Map<String, Object> update(Long id, Map<String, Object> updates) {
    Project project =
        projectRepository
            .findById(id)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Projet non trouvé"));

    if (updates.containsKey("title")) project.setTitle((String) updates.get("title"));
    ...
    if (updates.containsKey("status")) project.setStatus((String) updates.get("status"));
    if (updates.containsKey("supervisorId")) {
      Teacher supervisor =
          teacherRepository
              .findById(Long.parseLong((String) updates.get("supervisorId")))
              .orElseThrow(
                  () ->
                      new ResponseStatusException(HttpStatus.BAD_REQUEST, "Encadrant introuvable"));
      project.setSupervisor(supervisor);
  ...
  public void delete(Long id) {
    ...
    if (!juryRepository.findByProjectId(id).isEmpty()) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Impossible de supprimer ce projet car des jurys y sont rattachés");
    }
    ...
    if (slotAssignmentRepository.findAll().stream()
        .anyMatch(s -> project.getId().equals(s.getProjectId()))) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT,
