...
public class NotificationEventListener {
	...
	public void handleDomainEvent(DomainEvent event) {
		...
		if (event instanceof ProjectProposedEvent e) {
			...
			NotificationType type = e.getNewStatus().equalsIgnoreCase("APPROVED") ? NotificationType.SUCCESS : NotificationType.WARNING;
			notification = createNotification(
					type,
					"Statut du projet mis à jour",
					String.format("Le projet '%s' est passé de %s à %s", e.getProjectTitle(), e.getOldStatus(), e.getNewStatus()),
					"/coordinator/projects",
					e.getActor());
			...
			notification = createNotification(
					NotificationType.INFO,
					"Session créée",
					String.format("La session '%s' a été créée", e.getSessionName()),
					"/admin/sessions",
					e.getActor());
			...
			notification = createNotification(
					...
					"Session gelée",
					String.format("La session '%s' a été gelée. Les notes ne peuvent plus être modifiées.", e.getSessionName()),
					"/admin/sessions",
					e.getActor());
			...
			notification = createNotification(
					NotificationType.INFO,
					"Session dégelée",
					String.format("La session '%s' a été dégelée.", e.getSessionName()),
					"/admin/sessions",
					e.getActor());
			...
			notification = createNotification(
					NotificationType.SUCCESS,
					"Évaluation soumise",
					String.format("L'évaluation du projet '%s' a été soumise (Note: %.1f/20)", e.getProjectTitle(), e.getScore()),
					"/teacher/evaluations",
					e.getActor());
			...
			notification = createNotification(
					NotificationType.SUCCESS,
					"Session publiée",
					String.format("La session '%s' a été publiée.", e.getSessionName()),
					"/admin/sessions",
					e.getActor());
		} else if (event instanceof DefenseCancelledEvent e) {
			String dateStr = e.getDate() != null ? e.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) : "inconnue";
			String timeStr = e.getTime() != null ? e.getTime().format(DateTimeFormatter.ofPattern("HH:mm")) : "inconnue";
			notification = createNotification(
					NotificationType.WARNING,
					"Soutenance annulée",
					String.format("La soutenance du %s à %s a été annulée.", dateStr, timeStr),
	...
	private AppNotification createNotification(NotificationType type, String title, String message, String actionLink, String actor) {
