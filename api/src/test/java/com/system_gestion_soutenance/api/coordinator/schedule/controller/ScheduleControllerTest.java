...
class ScheduleControllerTest {
  ...
  void get_returnsSchedule() throws Exception {
    when(scheduleService.getSchedule())
        .thenReturn(Map.of("1", Map.of("id", 1L, "title", "Slot 1")));

    mockMvc
        .perform(get("/api/coordinator/schedule"))
  ...
  void autoGenerate_withDefenseSessionId_returnsSchedule() throws Exception {
    when(scheduleService.autoGenerate(1L))
        .thenReturn(Map.of("1", Map.of("title", "Generated Slot")));

    mockMvc
        .perform(
            post("/api/coordinator/schedule/auto-generate")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void publish_withDefenseSessionId_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/coordinator/schedule/publish")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(Map.of("defenseSessionId", "1"))))
        .andExpect(status().isOk())
