...
class ProjectControllerTest {
  ...
  void findAll_returnsProjects() throws Exception {
    ...
    mockMvc
        .perform(get("/api/coordinator/projects"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.size()").value(1))
        .andExpect(jsonPath("$[0].title").value("Projet Test"));
  }

  @Test
  void create_returnsCreated() throws Exception {
    CreateProjectRequest request =
        new CreateProjectRequest("New Project", "Description", 1L, List.of(1L), "PFE");
  ...
  void update_returnsProject() throws Exception {
    ...
    when(projectService.update(eq(1L), any()))
        .thenReturn(Map.of("id", 1L, "title", "Updated Project"));

    mockMvc
        .perform(
            put("/api/coordinator/projects/1")
                .contentType(MediaType.APPLICATION_JSON)
