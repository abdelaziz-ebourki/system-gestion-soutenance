...
@WebMvcTest(
    controllers = CoordinatorDefenseSessionController.class,
...
class CoordinatorDefenseSessionControllerTest {
  ...
  @MockitoBean private CoordinatorDefenseSessionService service;
  ...
  void findAll_returnsSessions() throws Exception {
    ...
    mockMvc
        .perform(get("/api/coordinator/defense-sessions"))
  ...
  void update_returnsSession() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/coordinator/defense-sessions/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void delete_returnsNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/coordinator/defense-sessions/1"))
  ...
  void transition_returnsUpdatedSession() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/coordinator/defense-sessions/1/transition")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(Map.of("toStatus", "ACTIVE"))))
        .andExpect(status().isOk())
