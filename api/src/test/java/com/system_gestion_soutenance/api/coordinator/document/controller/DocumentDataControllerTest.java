...
@WebMvcTest(
    controllers = DocumentDataController.class,
...
class DocumentDataControllerTest {
  ...
  @MockitoBean private DocumentDataService documentDataService;
    ...
    mockMvc
        .perform(
            post("/api/coordinator/documents/evaluation-sheets")
                ...
                .content(objectMapper.writeValueAsString(Map.of("projectId", 1))))
  ...
  void attendanceList_returnsData() throws Exception {
    when(documentDataService.attendanceList(1L))
        .thenReturn(Map.of("defenseSessionName", "Session PFE"));
  ...
  void procesVerbal_returnsData() throws Exception {
    when(documentDataService.procesVerbal(1L))
        .thenReturn(Map.of("studentNames", List.of("Jane Smith")));
